...
## Section B — `PerCoreCfg<F>` runtime bitmap cluster (permitted manual fields)

These fields appear inside `PerCoreCfg<F>` body AFTER the `FOREACH_PER_CORE_FIELD_TYPE` X-macro expansion. They are RUNTIME representations of `FOREACH_*_CFG_FLAG` bits, rebuilt from flat KIND_BOOL rows at slow-path rebuild (WIP2e adds the rebuild walker). Manual declaration justified by `alignas(8)` cluster boundary for atomic multi-byte bitmap reads.

CI script permits these 5 specific field names in `PerCoreCfg<F>` body. Any other manual field declaration in `PerCoreCfg<F>` body = BUILD ERROR.
