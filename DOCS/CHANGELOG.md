...
|---------|------|------------|
| 3.6.0 | 2026-04-02 | **Gate reason codes + structured logging + logging directory** — 13 gate reason codes (no_signal, no_trade, kill, cooldown, etc.) displayed in status bar, buy gate, and banner. Structured log output for trades, regime changes, gate state, session events. All runtime files moved to `logging/` directory (`rm -rf logging/*` for clean start). Kill switch debug diagnostics, balance drift false positive fix, chart trade markers disabled. GUI kill reset key (K). 259 assertions. |
