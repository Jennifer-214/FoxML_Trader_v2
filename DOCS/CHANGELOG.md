...
|---------|------|------------|
| 4.0.0 | 2026-04-25 | **Sharded as production + per-core ML + hot-swap** — Per-core sharded engine is now the production default (legacy single-threaded deprecated with runtime warning). Phase 5d zoo + 8a depth recorder + 8b notify + 8 maker/taker + 6prep confidence loop + 7prep validation all wired into sharded path (c5-c11). Per-core ConfidenceScorer with armed-but-inactive-on-noise-floor safety (c12-c16). Hot-swap strategy per core via GUI dropdown (waits for open position to close). Live Binance data is now the default — `build.sh` creates engine.cfg symlinks so `./engine` from any build dir connects to the real feed. 351/351 controller_test, 17/17 depth_recorder_test. See changelogs/2026-04-25-v4.0.0-sharded-production.md |
