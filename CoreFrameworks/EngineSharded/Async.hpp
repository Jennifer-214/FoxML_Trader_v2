...
//======================================================================
// MOVED (P4-pre-3c): EngineSharded_Async_DrainWithSubmit
//----------------------------------------------------------------------
// The drainer event pump now lives at EngineCommon.hpp as
// EngineCommon_DrainEventsAndSubmit, because the BACKTEST needs the same
// body and this header is live-plumbing (threads/adapter/GUI) the backtest
// must not include. D-441 had deleted EventLoop_OnEvent booking and wired
// only THIS converter, leaving the backtest with no pump at all.
// Callers pass the tick counter by value (live: ticks_produced.load()).
//======================================================================
