...
inline int Summary_WriteJson(const char* output_path,
    ...
    {
        bool first_field = true;
        json_emit_pair<FPN<F>>(f, "balance_start",     cfg.starting_balance,           first_field);
        json_emit_pair<FPN<F>>(f, "balance_end",       state.oms->balance,             first_field);
        json_emit_pair<FPN<F>>(f, "realized_pnl",      state.oms->realized_pnl,        first_field);
        json_emit_pair<FPN<F>>(f, "total_fees",        state.oms->total_fees,          first_field);
        json_emit_pair<FPN<F>>(f, "total_maker_fees",  state.oms->total_maker_fees,    first_field);
        json_emit_pair<FPN<F>>(f, "total_taker_fees",  state.oms->total_taker_fees,    first_field);
