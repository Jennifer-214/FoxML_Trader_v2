...
#define FOREACH_GLOBAL_CFG_FIELD(X)                                                                                                                                                                                  \
    ...
    X(uint8_t,              KIND_INT,        trading_mode,                "Trading Mode",         "Operational",     CfgFieldDescriptor::STAMP_BOUND | CfgFieldDescriptor::STAMP_BOUND_CFG_DERIVED | CfgFieldDescriptor::SAFETY_CRITICAL | CfgFieldDescriptor::HAS_SIDE_EFFECT | CfgFieldDescriptor::WARN_ON_CLAMP, INT(0, 0, 2), \
        "Trading mode + THE capital-authority field (NEW-1): 0=PAPER (default; safe), 1=LIVE (real money via REST API), 2=SHADOW (RESERVED/unimplemented — behaves as paper). Accepts string ('paper'/'live'/'shadow') or int. LIVE flips model_verify_strict 0->1 + reconcile_mode WARN->STRICT. HAS_SIDE_EFFECT — manual parser handles string form + NormalizeForMode triggers. SAFETY_CRITICAL (legacy use_real_money is a deprecated parse-alias that promotes trading_mode=live).", \
        STRAT_CAT_ALL,                                       OP_MODE_CAT_ALL, REGIME_CAT_ALL, RISK_CAT_ALL, CfgFieldDescriptor::STRUCT_CFG) \
    ...
    X(FPN_Binary<F>,               KIND_DOUBLE,     held_out_fraction,           "Held-Out %",           "Validation",      CfgFieldDescriptor::STAMP_BOUND | CfgFieldDescriptor::STAMP_BOUND_CFG_DERIVED | CfgFieldDescriptor::WARN_ON_CLAMP, DBL(0.20, 0.0, 1.0), \
        ...
        STRAT_CAT_ALL,                                       OP_MODE_CAT_ALL, REGIME_CAT_ALL, RISK_CAT_ALL, CfgFieldDescriptor::STRUCT_CFG) \
    /* v5.15.5.E.0.10 A6 ingress (D-221) — per-node majority-corrupt SHALT threshold (runtime safety policy; NOT stamp-bound). */ \
    X(FPN_Binary<F>,               KIND_DOUBLE,     model_corrupt_shalt_ratio,   "Corrupt SHALT Ratio",  "Drift Acknowledgments", CfgFieldDescriptor::WARN_ON_CLAMP, DBL(0.5, 0.0, 1.0), \
        "A6 ingress (D-221): per-node majority-SHALT threshold. When the fraction of ensemble arms with a CORRUPT barrier (popcount(corrupt_arms_mask)/primary_count) EXCEEDS this, the node SHALTs (refuses NEW trades + sticky 'model: CORRUPT — RETRAIN' alert). Default 0.5 = strict majority ('more corrupt than healthy halts'). A single corrupt arm below the ratio is dropped (graceful degradation) but still raises the alert; all-corrupt (empty barrier-zoo) always SHALTs.", \
        STRAT_CAT_ML,                                        OP_MODE_CAT_ALL, REGIME_CAT_ALL, RISK_CAT_ALL, CfgFieldDescriptor::STRUCT_CFG) \
...
#define FOREACH_PER_NODE_CFG_FIELD(X)                                                                                                                                                                                \
    ...
    X(Money, KIND_DOUBLE, fee_floor_mult,              "Fee Floor",            "Trading",         0,                                  DBL(3.0, 1.0, 20.0),                                                                                                             \
        ...
        STRAT_CAT_ALL,                                       OP_MODE_CAT_ALL, REGIME_CAT_ALL, RISK_CAT_ALL, CfgFieldDescriptor::STRUCT_CFG) \
    /* === Entry Filters (9) === */                                                                                                                                                                                   \
    X(Money, KIND_DOUBLE_PCT, entry_offset_pct,            "Offset %%",            "Entry Filters",   0,                                  DBL(0.15, 0.0, 5.0),                                                                                                            \
    ...
    X(Money, KIND_DOUBLE_PCT, offset_min,                  "Offset Min %%",        "Entry Filters",   0,                                  DBL(0.05, 0.0, 5.0),     "Adaptation lower bound for offset_pct",                                                          STRAT_CAT_STATIC_RULES | STRAT_CAT_REGRESSION_DRIVEN, OP_MODE_CAT_ALL, REGIME_CAT_ALL, RISK_CAT_ALL, CfgFieldDescriptor::STRUCT_CFG) \
    X(Money, KIND_DOUBLE_PCT, offset_max,                  "Offset Max %%",        "Entry Filters",   0,                                  DBL(0.50, 0.0, 5.0),     "Adaptation upper bound for offset_pct",                                                          STRAT_CAT_STATIC_RULES | STRAT_CAT_REGRESSION_DRIVEN, OP_MODE_CAT_ALL, REGIME_CAT_ALL, RISK_CAT_ALL, CfgFieldDescriptor::STRUCT_CFG) \
    ...
    /* === Momentum strategy (7) === */                                                                                                                                                                               \
    X(Money, KIND_DOUBLE, momentum_min_tp_margin_pct,  "Mom Min TP Margin",    "Strategies",      0,                                  DBL(0.0, 0.0, 0.05),     "Block momentum entry if TP too tight (0 = disabled; rec: 0.0040 = 0.40%)",                       STRAT_CAT_REGRESSION_DRIVEN,                                       OP_MODE_CAT_ALL, REGIME_CAT_ALL, RISK_CAT_ALL, CfgFieldDescriptor::STRUCT_CFG) \
    ...
    /* `bandit_blend_ratio` history, moved out of its TOOLTIP 2026-08-17 (D-426): it was a standalone   */ \
    /* model-const row until the .B.3 Step 1.6.2 cohort bit-add moved it here; the framework walker      */ \
    /* emits it unprefixed. The old prefixed wire key is now RETIRED and its name BURNED, and the        */ \
    /* burn sweep correctly flagged the tooltip for spelling it — string literals are exactly where a    */ \
    /* real resurrection would hide (a parser accepting the old key by name), so the sweep should NOT    */ \
    /* skip them. Archaeology belongs in a comment; the tooltip is operator-facing GUI text.             */ \
    X(FPN_Binary<F>                , KIND_DOUBLE, bandit_blend_ratio,          "Bandit Blend",         "ML",              CfgFieldDescriptor::STAMP_BOUND | CfgFieldDescriptor::STAMP_BOUND_CFG_DERIVED, DBL(0.5, 0.0, 1.0),      "Mix of bandit picks vs base model", STRAT_CAT_ML | STRAT_CAT_USES_BANDIT,                            OP_MODE_CAT_ALL, REGIME_CAT_ALL, RISK_CAT_ALL, CfgFieldDescriptor::STRUCT_CFG) \
    ...
    X(uint32_t              , KIND_INT, confidence_ic_floor_window,  "Conf IC Floor Window", "FoxML",           CfgFieldDescriptor::WARN_ON_CLAMP, INT(1000, 1, 100000),                                                              \
        ...
        STRAT_CAT_ML | STRAT_CAT_USES_CONFIDENCE,            OP_MODE_CAT_ALL, REGIME_CAT_ALL, RISK_CAT_ALL, CfgFieldDescriptor::STRUCT_CFG) \
    X(int                   , KIND_INT, confidence_ic_variant,       "Conf IC Variant",      "FoxML",           CfgFieldDescriptor::WARN_ON_CLAMP, INT(0, 0, 0),                                                                      \
        "IC variant: 0=Spearman (the ONLY registered variant; max tracks FOREACH_IC_VARIANT count-1 — bump when adding a variant, E.1.2.C A-2: out-of-range would compute constant-0 IC and poison drift-kill). Ships as KIND_INT pending TECH_DEBT-068 ML enum registry; promote to KIND_INT_ENUM after.",                                                                  \
        STRAT_CAT_ML | STRAT_CAT_USES_CONFIDENCE,            OP_MODE_CAT_ALL, REGIME_CAT_ALL, RISK_CAT_ALL, CfgFieldDescriptor::STRUCT_CFG) \
    ...
    X(Money, KIND_DOUBLE, partial_exit_pct,            "Partial Exit %%",      "Partial Exits",   0,                                  DBL(0.5, 0.0, 1.0),      "Fraction of position to close at TP1 (0.5 = 50%)",                                               STRAT_CAT_ALL,                                                   OP_MODE_CAT_ALL, REGIME_CAT_ALL, RISK_CAT_ALL, CfgFieldDescriptor::STRUCT_CFG) \
    X(Money, KIND_DOUBLE, tp2_mult,                    "TP2 Mult",             "Partial Exits",   0,                                  DBL(2.0, 0.0, 10.0),     "TP2 distance = TP1_distance * this (2.0 = double TP)",                                           STRAT_CAT_ALL,                                                   OP_MODE_CAT_ALL, REGIME_CAT_ALL, RISK_CAT_ALL, CfgFieldDescriptor::STRUCT_CFG) \
    ...
    X(int                   , KIND_INT, barrier_blend_mode,          "Barrier Blend Mode",   "ML",              CfgFieldDescriptor::STAMP_BOUND | CfgFieldDescriptor::STAMP_BOUND_CFG_DERIVED | CfgFieldDescriptor::HAS_SIDE_EFFECT | CfgFieldDescriptor::WARN_ON_CLAMP, INT(0, 0, 3),                                  \
        "Per-horizon TP/SL serving mode: 0=LEGACY (cfg-direct fallback), 1=BLEND (weighted across horizons), 2=DOMINANT (highest-weight horizon). HAS_SIDE_EFFECT — manual parser handles string form ('legacy'/'blend'/'dominant'). .B.3 Step 1.6.2 cohort bit-add (Decision D mechanism 1; the legacy inference_cfg_barrier_blend_mode POST_CFG row was NOT deleted — it survives in the MODEL_CONST registry with no production producer; PARITY-042 family, tombstone decision pending — E.1.2.C leg 2 comment-truth fix).", \
        STRAT_CAT_ML,                                                   OP_MODE_CAT_ALL, REGIME_CAT_ALL, RISK_CAT_ALL, CfgFieldDescriptor::STRUCT_CFG) \
