...
//======================================================================================================
// Bit-packed observability state for PerCoreSnap. Distinct from the
...
//   3. PerCoreSnap struct field (uint16_t state_flags) — declared once
//   4. Setter / reader sites (writer toggles via BITMAP_SET/CLR/IS_SET;
//      readers via BITMAP_IS_SET — placement contextual per writer)
//
// CLOSES TECH_DEBT-013 candidate (3) — PerCoreSnap non-failure state flags.
...
//   1. Append X(NAME, "doc") to FOREACH_PER_CORE_STATE_FLAG
...
#ifndef PER_CORE_STATE_FLAGS_REGISTRY_HPP
#define PER_CORE_STATE_FLAGS_REGISTRY_HPP
...
#define FOREACH_PER_CORE_STATE_FLAG(X)                                                              \
    /* Migrated from PerCoreSnap.permission (v5.6.1) — 0=forbidden, 1=allowed */                   \
    X(PERMISSION_ALLOWED,                                                                            \
      "entries allowed for this core (post-warmup; no kill-switch trip)")                           \
    /* Migrated from PerCoreSnap.bitmap_consistency (v5.6.1) — display↔execution invariant */      \
    X(BITMAP_CONSISTENT,                                                                             \
      "(positions[i].idx >= 0) == (active|active_b) — display↔execution invariant")                 \
    /* Migrated from PerCoreSnap.gate_direction — 0=buy below, 1=buy above */                       \
    X(GATE_BUY_ABOVE,                                                                                \
      "buy direction: bit set = buy above (MOM); bit clear = buy below (MR/DIP/EMA/ML)")            \
    /* Migrated from PerCoreSnap.is_ml — 1=STRATEGY_ML core */                                      \
    X(IS_ML,                                                                                         \
      "STRATEGY_ML core with ML extras valid (ml_* fields populated)")                              \
    /* Migrated from PerCoreSnap.ml_model_loaded — 1=zoo has at least one role loaded */            \
    X(ML_MODEL_LOADED,                                                                               \
      "zoo has at least one role loaded; non-zero prediction expected")                             \
    /* Migrated from PerCoreSnap.strategy_was_explicit_set (v5.9.0c) — cfg explicit-set bit */     \
...
enum PerCoreStateFlag {
    FOREACH_PER_CORE_STATE_FLAG(X_GEN_STATE_FLAG_BIT)
    PER_CORE_STATE_FLAG_COUNT  // sentinel
};
...
FOREACH_PER_CORE_STATE_FLAG(X_GEN_STATE_FLAG_MASK)
#undef X_GEN_STATE_FLAG_MASK

static_assert(PER_CORE_STATE_FLAG_COUNT <= 16,
              "PerCoreSnap state_flags uint16_t exhausted; expand to uint32_t");

// Public count macro for tests (uses >= per /readiness Check 21).
#define X_GEN_STATE_FLAG_COUNT_ONE(name, doc) +1
#define FOREACH_PER_CORE_STATE_FLAG_COUNT (0 FOREACH_PER_CORE_STATE_FLAG(X_GEN_STATE_FLAG_COUNT_ONE))
...
#endif  // PER_CORE_STATE_FLAGS_REGISTRY_HPP
