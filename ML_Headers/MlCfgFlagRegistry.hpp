...
#define FOREACH_ML_CFG_FLAG(X)                                                                                                                                                                          \
    ...
    X(BANDIT_ENABLED,               bandit_enabled,               "Bandit",                "FoxML",       0,                                                "Exp3/Thompson bandit drives BUY-side ensemble arm weighting (default ON since 2026-08-16). Gates SELECT only — with it off the bandit still learns from real outcomes but does not drive decisions, so turning it on starts from warm posteriors. Algorithm choice is cfg.bandit_algorithm (0=Exp3, else Thompson). Exit side has its own flag: exit_bandit_enabled") \
