...
class FoxmlChart:
    def __init__(self, accumulator, trade_reader):
        ...
        self.fig.text(0.02, 0.97, 'foxml trader', fontsize=12,
                      color=C['peach'], fontweight='bold', va='top')
    ...
    def render(self):
        ...
        for i in range(n):
            ...
            wick_color = C['text'] if closes[i] >= opens[i] else C['dim']
        ...
        if n > 10:
            ...
            self.ax_price.plot(xs, avg, color=C['clay'], linewidth=1, alpha=0.7)
        ...
        for i in range(n):
            ...
            color = C['green'] if ratio > 0.5 else C['red']
        ...
        if self.trades.equity:
            ...
            self.ax_equity.plot(eq_xs, eq_vals, color=C['wheat'], linewidth=1.5)
            self.ax_equity.axhline(y=0, color=C['surf'], linewidth=1, linestyle='-')
