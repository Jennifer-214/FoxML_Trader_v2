...
//   - train_acc - cv_acc >= 0.20  → train/CV gap (in-sample vs
//   cross-validation)
//   - train_acc - val_acc >= 0.20 → train/val gap (in-sample vs held-out)
//   - n_features >= cap           → feature count cap (optional, prevents curse
//   of dimensionality)
//
...
OverfitDetection_Check(float train_acc, float cv_acc, float val_acc,
  ...
  if (train_acc >= acc_thresh) {
    r.is_overfit = 1;
    r.check_failed = 1;
    snprintf(r.reason, sizeof(r.reason), "memorization: train_acc %.4f >= %.2f",
             train_acc, acc_thresh);
    return r;
  }
  ...
  if (cv_acc >= 0.0f) {
    ...
    if (gap >= gap_thresh) {
      ...
      snprintf(r.reason, sizeof(r.reason),
               "train/CV gap: %.4f >= %.2f (train=%.4f, cv=%.4f)", gap,
               gap_thresh, train_acc, cv_acc);
      return r;
    }
  }
  ...
  if (val_acc >= 0.0f) {
    ...
    if (gap >= gap_thresh) {
      ...
      r.check_failed = 3;
      snprintf(r.reason, sizeof(r.reason),
               "train/val gap: %.4f >= %.2f (train=%.4f, val=%.4f)", gap,
               gap_thresh, train_acc, val_acc);
      return r;
    }
  }
  ...
  if (feat_cap > 0 && n_features >= feat_cap) {
    r.is_overfit = 1;
    r.check_failed = 4;
    snprintf(r.reason, sizeof(r.reason), "feature count: %d >= %d", n_features,
             feat_cap);
    return r;
  }
...
static inline OverfitReport OverfitDetection_CheckDefaults(float train_acc,
                                                           float cv_acc,
                                                           float val_acc,
                                                           int n_features) {
  return OverfitDetection_Check(
...
#define OVERFIT_TRAIN_CORR_THRESHOLD                                           \
  0.99f                                  // |train_corr| >= this = memorization
#define OVERFIT_TRAIN_VAL_CORR_GAP 0.20f // train_corr - val_corr >= this = gap

static inline OverfitReport
OverfitDetection_CheckRegression(float train_corr, float val_corr,
                                 int n_features, float corr_thresh,
                                 float gap_thresh, int feat_cap) {
  OverfitReport r;
  memset(&r, 0, sizeof(r));
  // we reuse the OverfitReport struct — fields hold correlations for
  // regression. train_accuracy = train_corr, val_accuracy = val_corr,
  // train_val_gap = corr gap. The display layer reads
  // WalkForwardResults.label_kind to format these.
...
static inline OverfitReport
OverfitDetection_CheckRegressionDefaults(float train_corr, float val_corr,
                                         int n_features) {
  return OverfitDetection_CheckRegression(
      train_corr, val_corr, n_features, OVERFIT_TRAIN_CORR_THRESHOLD,
      OVERFIT_TRAIN_VAL_CORR_GAP, OVERFIT_FEATURE_COUNT_CAP);
}

// aggregate overfit status across multiple folds
// returns: number of folds flagged as overfit
static inline int OverfitDetection_CountOverfit(const OverfitReport *reports,
                                                int n_folds) {
  int count = 0;
  for (int i = 0; i < n_folds; i++) {
    if (reports[i].is_overfit)
      count++;
  }
  return count;
}
...
static inline void OverfitDetection_Print(const OverfitReport *r,
                                          int fold_idx) {
  if (r->is_overfit) {
    fprintf(stderr, "[overfit] fold %d: OVERFIT — %s\n", fold_idx + 1,
            r->reason);
  } else {
    fprintf(stderr, "[overfit] fold %d: clean (train=%.4f", fold_idx + 1,
            r->train_accuracy);
    if (r->cv_accuracy >= 0.0f)
      fprintf(stderr, ", cv=%.4f, gap=%.4f", r->cv_accuracy, r->train_cv_gap);
    if (r->val_accuracy >= 0.0f)
      fprintf(stderr, ", val=%.4f, gap=%.4f", r->val_accuracy,
              r->train_val_gap);
    fprintf(stderr, ")\n");
  }
}
